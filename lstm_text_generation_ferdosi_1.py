...
text = open('trainset/ferdosi_poem_1.txt', encoding="utf8").read()
...
for iteration in range(2):
    ...
    model.fit(x, y, batch_size=128, epochs=500) # why epochs=?
    ...
    for diversity in [0.2, 0.5, 1.0, 1.2, 1.5]:
        ...
        file_name = 'result_ferdosi/' + 'step' + repr(step) \
        + '_itr' + repr(iteration) + '_div' + repr(diversity) + '.txt'
        with open(file_name, 'w',  encoding='utf8') as generated_text_file:
             generated_text_file.write(generated)
        print()
        

# Save model (requires HDF5 and h5py)
model.save('lstm_text_generation_ferdosi'+ dt + '.h5')
